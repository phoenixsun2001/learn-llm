...
python run_pipeline.py --process-only       # Re-classify existing materials + rebuild index
...
PYTHONPATH="." python tests/test_summarizer.py     # summarizer fallback (no-LLM)
PYTHONPATH="." python tests/test_classifier.py     # classifier keyword fallback
PYTHONPATH="." python tests/test_llm_client.py     # LLM backend priority + None fallback
...
- REST API routes (content production): `routes/review.py`, `routes/materials.py`, `routes/sources.py`, `routes/chat.py`. CRUD for tutorials/pathways/scenarios/prompts/tools/skills is frontend-only (localStorage) — see `frontend/src/data/admin-config.js`.
